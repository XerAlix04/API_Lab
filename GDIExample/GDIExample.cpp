...
{
    static HDC hdc;
    static COLORREF blue;
    static int width, height;
    static HPEN hPenGreen, hPenBlack, hPenYellow;
    static HBRUSH hBrushViolet, hBrushLightBlue;
    switch (message)
    {
    case WM_SIZE:
        //Lấy lại kích thước cửa sổ
    ...
    case WM_LBUTTONDOWN:
        ...
        blue = RGB(0, 0, 255);
        SetPixel(hdc, LOWORD(lParam), HIWORD(lParam), blue);
        ReleaseDC(hWnd, hdc);
        break;
    case WM_COMMAND:
        {
    ...
    case WM_PAINT:
        {
            ...
            hPenYellow = CreatePen(PS_SOLID, 3, RGB(255, 255, 0));
            ...
            Rectangle(hdc, width / 8, height / 8, width - (width / 8), height - (height / 8));
            ...
            Ellipse(hdc, width / 8, height / 8, width - (width / 8), height - (height / 8));

            SelectObject(hdc, hBrushViolet);
            RoundRect(hdc, width / 4, height / 4, width - (width / 4), height - (height / 4), width / 5, height / 5);
